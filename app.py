...
def process_excel_data(uploaded_template_file, uploaded_input_file):
    ...
    for cell_range_str in list(ws.merged_cells.ranges):
        min_col_mc, min_row_mc, max_col_mc, max_row_mc = openpyxl.utils.get_column_letter(openpyxl.utils.cell.range_boundaries(str(cell_range_str)))
        if (
           (min_row_mc <= YEAR_ROW <= max_row_mc) or            (min_row_mc <= MONTH_ROW <= max_col_mc) or            (min_row_mc <= START_ROW_DATA <= max_row_mc) or            (min_row_mc <= YEARLY_TOTAL_ROW <= max_row_mc)
        ):
...
input_file = st.file_uploader("Παρακαλώ ανεβάστε το INPUT excel (μόνο 2 στήλες) - (CV - ΑΡΩΝΗΣ.xlsx)", type=["xlsx"])

if template_file is None:
    # Attempt to load default template if not uploaded
    try:
        ...
        st.warning("Παρακαλώ ανεβάστε ένα αρχείο template ή βεβαιωθείτε ότι το 'AM TEST 1.xlsx' υπάρχει στον ίδιο φάκελο.")
        template_file_bytes = None
else:
    template_file_bytes = io.BytesIO(template_file.read())


if input_file and template_file_bytes:
